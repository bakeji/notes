...
export default function NoteNav({saveNote, archiveNotes, restoreNotes, deleteNote}: NoteNavProps){
    const {setShowClickedNote, showArchivedNote,} = useNoteContext()
    const archOrRestNote =showArchivedNote? restoreNotes: archiveNotes
     return(
        <div className={`hidden  items-center justify-between mb-5 py-3 border-b max-lg:flex`}>
            <button onClick={()=>setShowClickedNote(false)} className="flex items-center font-inter text-[14px] font-[400]"> <Image width={18} height={18} src='/left.png' alt="go back"/>  Go back</button>
            <div className="flex items-center justify-center gap-3">
                            
                <AlertDialog>
                    <AlertDialogTrigger className=" border-none font-inter text-[14px] font-[500] text-[#0E121B]">
