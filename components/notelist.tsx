        ...
        <div className={`w-[25%] border-r  border-solid ${showSelectedSetting ||isNewNote?'max-lg:hidden' : 'max-lg:block'}  max-lg:flex max-lg:flex-col max-lg:justify-between max-lg:w-[90%]   max-lg:border-none ${showClickedNote||showAllTags? 'max-lg:hidden': ''}`}>
           ...
           {selectedTag && <button onClick={()=>setShowAllTags(true)} className={`items-center  mb-3 font-inter text-[14px] font-[400] hidden max-lg:flex`}> <Image width={18} height={18} src='/left.png' alt="go back"/>  Go back</button>}
            {!showSearchResult && <h1 className={`hidden max-lg:flex font-inter font-[700] text-[24px] ${selectedTag? 'text-[#525866]' : 'text-[#0E121B]'} `}>{showAllNote? 'All Notes': showArchivedNote? "Archived Notes": `Notes Tagged:`} <span className="text-[#0E121B]">{selectedTag}</span></h1>}
               {selectedTag!=="" && <p className={`font-inter text-[14px] font-[400] text-[#2B303B] ${theme==='Dark Mode'? ' text-white  ': ''}`}>{`All notes with the ”${selectedTag}” tag are shown here.`}</p> }
               {showArchivedNote && <p className={`font-inter text-[14px] font-[400] text-[#2B303B] ${theme==='Dark Mode'? ' text-white  ': ''}`}>All your archived notes are stored here. You can restore or delete them anytime.</p>}
               {showArchivedNote && notesArray.length < 1  && <div className={` bg-[#F3F5F8] rounded-[6px] w-[100%] border-[1px] border-solid border-[#E0E4EA] p-[2px] ${theme==='Dark Mode'? 'bg-[#232530]':'' }`}>
                    <p>No notes have been archived yet. Move notes here for safekeeping, or create a new note.</p>
                </div>}
              {showSearchResult && searchResults.length <1 &&  <div className={` ${theme==='Dark Mode'? 'bg-[#232530] text-white ':'' }bg-[#F3F5F8] rounded-[6px] w-[100%] border-[1px] border-solid border-[#E0E4EA] p-[1px] `}>
                ...
                <div key={index} className={`w-[100%] border-t-[1px] ${theme==='Dark Mode'? 'text-white':'' } border-solid p-[2px] ${selectedNotes && selectedNotes === note? "bg-[#F3F5F8] rounded-[6px] border-none max-lg:bg-transparent": ''}`} >
                    <p onClick={()=>{handleNoteClick(index)}} className=" cursor-pointer font-inter text-[16px] font-[600]  lin">{note?.title}</p>
